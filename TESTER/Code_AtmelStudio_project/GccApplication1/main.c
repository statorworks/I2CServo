...
void StateManager(void){
 ...
 //Check change in power pot
  ...
  //
  data_out[0] = 2; //<<We need park type always as 'hold' for testing
  data_out[1] = 1; //default cw motor polarity
  data_out[2] = 0; //default no continuous rotation
  data_out[3] = max_power;  
  data_out[4] = max_speed>>8;
  data_out[5] = max_speed;
  data_out[6] = ramp_time>>8;
  data_out[7] = ramp_time;      
  data_out[8] = ramp_curve; 
  I2C_MasterWrite(0, address, REG_PARK_TYPE, data_out, 9);	 
